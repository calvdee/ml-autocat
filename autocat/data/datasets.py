...
def get_training_data(data_path, filters=None, drop_na=False):
...
def get_totals_data(path=None):
    """ Loads the product totals data to use with the financial scorer.
    Assumes that the data will have (index,UniversalProductCode,SumNetAmount) headers.
    
    Parameters
    ----------
    path: string
        The path to the product totals data.  If no path is given, the default will
        be used.
    Returns
    -------
    pandas.DataFrame
        The totals data with an index named 'index'
    """
    import pandas as pd
    from ..settings import TOTALS_DATA
    
    if path is None:
        totals_data = pd.read_csv(TOTALS_DATA).set_index('index')
    else:
        totals_data = pd.read_csv(path).set_index('index')
